...
class Sketch(CanvasBase):
    ...
    # Add the selected_components attribute here
    selected_components = []

    def __init__(self, parent):
        super(Sketch, self).__init__(parent)
        #added dictionary to index components by name
        self.cDict = {}
    ...
    def Interrupt_Keyboard(self, keycode):
        ...
        # Create a dictionary to hold your creature's components
        self.cDict = {
            "head": self.components[0],  # Assuming the head is the first component
            "body": self.components[1],  # Assuming the body is the second component
            "leg1": self.components[2],  # Assuming leg1 is the third component
            "leg2": self.components[3],  # Assuming leg2 is the fourth component
            "leg3": self.components[4],  # Assuming leg3 is the fifth component
            "leg4": self.components[5]   # Assuming leg4 is the sixth component
        }
        #poses
        if keycode == wx.WXK_SPACE:
            # Pose 1: Reset to the default position
            for component in self.components:
                component.reset()

        if keycode == ord('1'):
            # Pose 2: Define a specific pose by rotating limbs
            # Example: Rotate leg1 to 45 degrees
            if len(self.components) > 0:
                self.components[1].rotate(45, self.components[1].axisBucket[0])
            # Update other limb rotations as needed

        if keycode == ord('2'):
            # Pose 3: Another specific pose
            # Example: Rotate leg2 to -30 degrees
            if len(self.components) > 1:
                self.components[2].rotate(-30, self.components[2].axisBucket[0])
            # Update other limb rotations as needed

        if keycode == ord('3'):
            # Pose 4: Another specific pose
            # Example: Rotate leg3 to 60 degrees
            if len(self.components) > 2:
                self.components[3].rotate(60, self.components[3].axisBucket[0])
            # Update other limb rotations as needed

        if keycode == ord('4'):
            # Pose 5: Another specific pose
            # Example: Rotate leg4 to -45 degrees
            if len(self.components) > 3:
                self.components[4].rotate(-45, self.components[4].axisBucket[0])
        ...
        if chr(keycode) in "1234567890ABCDEFGHIJKLMNOPQRSTUVWXYZ":
            # Get the index of the component to control based on the pressed key
            component_index = "1234567890ABCDEFGHIJKLMNOPQRSTUVWXYZ".index(chr(keycode))

            # Toggle selection for the component (add or remove from selection)
            if component_index in self.selected_components:
                self.selected_components.remove(component_index)
            else:
                self.selected_components.append(component_index)

            # Update the color of selected components
        ...
        if keycode in [wx.WXK_UP]:
            # Increase rotation angle for all selected components
            rotation_speed = 1  # Adjust the rotation speed as needed
            for index in self.selected_components:
                self.components[index].rotate(rotation_speed, self.components[index].axisBucket[self.select_axis_index])
            self.update()
        if keycode in [wx.WXK_DOWN]:
            # Decrease rotation angle for all selected components
            rotation_speed = -1  # Adjust the rotation speed as needed
            for index in self.selected_components:
                self.components[index].rotate(rotation_speed, self.components[index].axisBucket[self.select_axis_index])
            self.update()
